...
<div class="table-list comment">
	...
	if(is_array($data)) {
		foreach($data as $v) {
			$comment_info = $this->comment_db->getby_commentid($v['commentid']);
	...
	?>
     <tr id="tbody_<?php echo $v['id']?>">
		...
		<td width="230"><a href="?app=comment&controller=comment_admin&action=listinfo&search=1&searchtype=0&keyword=<?php echo urlencode($comment_info['title'])?>&tableid=<?php echo $tableid?>"><?php echo $comment_info['title']?></td>
...
function show_tbl(obj) {
	var pdoname = $(obj).val();
	location.href='?app=comment&controller=comment_admin&action=listinfo&tableid='+pdoname;
}
