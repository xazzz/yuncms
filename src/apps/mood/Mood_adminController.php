 ...
 *          85825770@qq.com $
 */
class Mood_adminController extends Web_Admin {
	...
	public function setting() {
		if (isset ( $_POST ['dosubmit'] )) {
			...
			$data = array ();
			foreach ( $name as $k => $v ) {
				$data [$k] = array ('use' => isset($use [$k]) ? $use [$k] : 0,'name' => $v,'pic' => $pic [$k] );
			}
			S ( 'common/mood_program', $data );
			showmessage ( L ( 'operation_success' ), HTTP_REFERER );
		} else {
			$mood_program = S ( 'common/mood_program' );
			include $this->view ( 'mood_setting' );
		}
	}
}