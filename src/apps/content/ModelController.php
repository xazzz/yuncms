 ...
 * @version $Id: ModelController.php 675 2013-07-30 03:40:40Z 85825770@qq.com $
 */
...
class ModelController extends Web_Admin {
	...
	public function init() {
		...
		include $this->view ( 'model_manage' );
	}
	...
	public function add() {
		if (isset ( $_POST ['dosubmit'] )) {
			...
			$modelid = $this->db->insert ( $_POST ['info'],true );
		...
		foreach ( $classtypes as $classtype ) {
			...
			foreach ( $fields as $field => $fieldvalue ) {
				if (file_exists ( MODEL_PATH . $field . DIRECTORY_SEPARATOR . $classtype . '.inc.php' )) {
					...
					$cache_data .= String::substr_between($cache_data_info,'<?php','?>');
				}
			}
			$cache_data .= "\r\n } \r\n?>";
