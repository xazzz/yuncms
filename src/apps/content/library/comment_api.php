...
if (! app_exists ( 'comment' )) showmessage ( L ( 'application_not_exists' ) );
class comment_api {
	private $db;
	...
	function get_info($module, $contentid) {
		...
		$r = $this->db->where ( array ('catid' => $catid,'id' => $contentid ))->field( 'title' )->find();
		...
		if ($cat ['type'] == 0) {
			if ($model [$cat ['modelid']] ['tablename']) {
				...
				$data_info = $this->db->getby_id(  $contentid  );
			}
		}
		if ($r) {
