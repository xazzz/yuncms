 ...
 *          85825770@qq.com $
 */
class Member_verifyController extends Web_Admin {
    ...
    public  function manage() {
        ...
        include $this->view ( 'member_verify' );
    }
    ...
    public function modelinfo() {
        ...
        include $this->view ( 'member_verify_modelinfo' );
    }
    ...
    function pass() {
        if (isset ( $_POST ['userid'] )) {
            ...
            $userarr = $this->db->where(array('userid'=>array('in',$userids)))->listinfo ();
            ...
            $userids = implode(',', $success_uids);
            $where = array('userid'=>array('in',$userids));
            $this->db->where($where)->update ( array ('status' => 1,'message' => $_POST ['message'] ) );

            $fail_uids = array_diff ( $uidarr, $success_uids );
            if (! empty ( $fail_uids )) {
            	$userids = implode(',', $fail_uids);
            	$where = array('userid'=>array('in',$userids));
                $this->db->where($where)->update ( array ('status' => 5,'message' => $_POST ['message'] ));
            }
            // 发送 email通知
            if ($_POST ['sendemail']) {
                $memberinfo = $this->db->where($where)->select (  );
    ...
    function reject() {
        if (isset ( $_POST ['userid'] )) {
            ...
            $userids = implode(',', $uidarr);
            $where = array('userid'=>array('in',$userids));
            $res = $this->db->where($where)->update ( array ('status' => 4,'message' => $_POST ['message'] ) );
            if ($res) {
                if ($_POST ['sendemail']) {
                    $memberinfo = $this->db->where($where)->select (  );
                    foreach ( $memberinfo as $v ) {
    ...
    function ignore() {
        if (isset ( $_POST ['userid'] )) {
            $uidarr = isset ( $_POST ['userid'] ) ? $_POST ['userid'] : showmessage ( L ( 'illegal_parameters' ), HTTP_REFERER );
             $userids = implode(',', $uidarr);
            $where = array('userid'=>array('in',$userids));
            $res = $this->db->where($where)->update ( array ('status' => 2,'message' => $_POST ['message'] ) );
            // 发送 email通知
            if ($res) {
                if ($_POST ['sendemail']) {
                    $memberinfo = $this->db->where($where)->select (  );
