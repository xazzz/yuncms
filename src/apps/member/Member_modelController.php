 ...
 *          85825770@qq.com $
 */
class Member_modelController extends Web_Admin {
	...
	public function manage() {
		...
		include $this->view ( 'member_model_list' );
	}
	...
	public function add() {
		if (isset ( $_POST ['dosubmit'] )) {
			...
			include $this->view ( 'member_model_add' );
		}
	}
	...
	public function edit() {
		if (isset ( $_POST ['dosubmit'] )) {
			...
			include $this->view ( 'member_model_edit' );
		}
	}
	...
	public function delete() {
		...
		$modelids = implode ( ',', $modelidarr );
		$where = array('modelid'=>array('in',$modelids));
		...
		if ($this->db->where($where)->delete (  )) {
			// 删除模型字段
			$this->model_field_db = Loader::model ( 'model_field_model' );
			$this->model_field_db->where($where)->delete (  );
			// 修改用户模型组为普通会员
			$this->member_db = Loader::model ( 'member_model' );
			$this->member_db->where($where)->update ( array ('modelid' => 4 ) );
			// 更新模型缓存
			Loader::lib ( 'member:member_cache', false );
			member_cache::update_cache_model ();
	...
	public function move() {
		if (isset ( $_POST ['dosubmit'] )) {
			...
			$show_header = $show_scroll = true;
			$modelarr = $this->db->where(array ('type' => 2 ))->select ( );
			foreach ( $modelarr as $v ) {
				$modellist [$v ['modelid']] = $v ['name'];
			}
			include $this->view ( 'member_model_move' );
		}
	}
	...
	public function public_checkmodelname_ajax() {
		...
		$status = $this->db->getby_name ( $modelname );
		if ($status) exit ( '0' );
		else exit ( '1' );
	}
