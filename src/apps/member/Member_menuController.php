 ...
 * @version $Id: Member_menuController.php 677 2013-07-30 03:43:44Z 85825770@qq.com $
 */
class Member_menuController extends Web_Admin {
    ...
    public function add() {
        if(isset($_POST['dosubmit'])) {
            ...
            //开发过程中用于自动创建语言包
            $file = WEKIT_PATH.'languages'.DIRECTORY_SEPARATOR.'zh-cn'.DIRECTORY_SEPARATOR.'member_menu.lang.php';
            if(file_exists($file)) {
            ...
            foreach($result as $r) {
                $r['cname'] = L($r['name'], '', 'member_menu');
                $r['selected'] = $r['id'] == isset($_GET['parentid']) ? 'selected' : '';
                $array[] = $r;
            }
    ...
    public function edit() {
        if(isset($_POST['dosubmit'])) {
            ...
            //修改语言文件
            $file = WEKIT_PATH.'languages'.DIRECTORY_SEPARATOR.'zh-cn'.DIRECTORY_SEPARATOR.'member_menu.php';
            require $file;
            $key = $_POST['info']['name'];
            if(!isset($LANG[$key])) {
                $content = File::read($file);
