...
class AttachmentsController {
	...
	public function album_load() {
		...
		if (isset ( $_GET ['dosubmit'] )) {
			...
			if ($uploadtime) {
				...
				$where['uploadtime'] = array('elt',$stop_uploadtime);
			}
		}
		$page = isset ( $_GET ['page'] ) ? $_GET ['page'] : 1;
		$infos = $this->att_db->where($where)->order('aid DESC')->listinfo ($page, $pagesize = 8);
	...
	public function album_dir() {
		...
		$local = str_replace ( array (WEKIT_PATH,BASE_PATH,DIRECTORY_SEPARATOR . DIRECTORY_SEPARATOR ), array ('','',DIRECTORY_SEPARATOR ), $filepath );
		$url = ($dir == '.' || $dir == '') ? $this->upload_url : $this->upload_url . str_replace ( '.', '', $dir ) . '/';
		$show_header = true;
		include $this->view ( 'album_dir' );
	}
	...
	final public static function view($file, $app = '') {
		 $app = empty( $app) ? APP :  $app;
		if(empty( $app)) return false;
		return APPS_PATH.$app.DIRECTORY_SEPARATOR.'view'.DIRECTORY_SEPARATOR.$file.'.tpl.php';
	}
}