 ...
 * @version $Id: UeditorController.php 668 2013-07-30 03:08:49Z 85825770@qq.com $
 */
...
class UeditorController {
	...
	public function upimg() {
		...
		if ($aids [0]) {
			...
			$return = array ('url' => $this->upload_url . $attachment->uploadedfiles [0] ['filepath'],'title' => $title,'original' => $oriname,'state' => 'SUCCESS' );
		} else {
	...
	public function scrawl() {
		$grouplist = S ( 'member/grouplist' );
		if ($_GET ['swf_auth_key'] != md5 ( C ( 'config', 'auth_key' ) . $_GET ['SWFUPLOADSESSID'] ) || ($_GET ['isadmin'] == 0 && ! $grouplist [$_GET ['groupid']] ['allowattachment'])) exit ();
		$application = trim ( $_GET ['application'] );
		$catid = intval ( $_GET ['catid'] );
		$attachment = new Attachment ( $application, $catid );
		...
		if (isset ( $_GET ['act'] ) && $_GET ['act'] == "tmpImg") { // 背景上传
			...
			if ($aids [0]) {
				...
				Folder::rm($this->upload_path . "tmp/");// 上传成功后删除临时目录
