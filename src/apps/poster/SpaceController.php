 ...
 * @version $Id: SpaceController.php 648 2013-07-29 09:56:28Z 85825770@qq.com $
 */
defined ( 'IN_YUNCMS' ) or exit ( 'No permission resources.' );

class SpaceController extends Web_Admin {
	private $M, $db;
	...
	public function public_preview() {
		if (is_numeric ( $_GET ['spaceid'] )) {
			...
			include $this->view ( 'space_preview' );
		}
	}
	...
	public function poster_template() {
		$templatedir = WEKIT_PATH . 'template' . DIRECTORY_SEPARATOR . C ( 'template', 'name' ) . DIRECTORY_SEPARATOR . 'poster' . DIRECTORY_SEPARATOR;
		$poster_template = S ( 'common/poster_template' );
		...
		$big_menu = big_menu ( U ( 'poster/space/add' ), 'add', L ( 'add_space' ), 540, 320 );
		include $this->view ( 'poster_template' );
	}
	...
	public function public_tempate_setting() {
		...
		if (isset ( $_POST ['dosubmit'] )) {
			...
			include $this->view ( 'template_setting' );
		}
	}
