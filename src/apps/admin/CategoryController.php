 ...
 * @version $Id: CategoryController.php 656 2013-07-30 02:01:22Z 85825770@qq.com $
 */
defined ( 'IN_YUNCMS' ) or exit ( 'No permission resources.' );
error_reporting ( E_ERROR );
class CategoryController extends Web_Admin {
	private $db;
	...
	public function init() {
		...
		if (! empty ( $result ) && is_array ( $result )) {
			foreach ( $result as $r ) {
				...
				$r ['str_manage'] .= '<a href="?app=admin&controller=category&action=add&parentid=' . $r ['catid'] . '&menuid=84&s=' . $r ['type'] . '">' . L ( 'add_sub_category' ) . '</a> | ';
		...
		include $this->view ( 'category_manage' );
	}
	...
	public function add() {
		if (isset ( $_POST ['dosubmit'] )) {
			...
			if ($type == 0) {
				...
				include $this->view ( 'category_add' );
			} elseif ($type == 1) {
				include $this->view ( 'category_page_add' );
			} else {
				include $this->view ( 'category_link' );
			}
		}
	}
	...
	public function edit() {
		if (isset ( $_POST ['dosubmit'] )) {
			...
			$type = $_GET ['type'];
			if ($type == 0) {
				include $this->view ( 'category_edit' );
			} elseif ($type == 1) {
				include $this->view ( 'category_page_edit' );
			} else {
				include $this->view ( 'category_link' );
			}
		}
	...
	private function repair() {
		...
		if (is_array ( $this->categorys )) {
			foreach ( $this->categorys as $catid => $cat ) {
				...
				$this->sethtml = isset($setting ['create_to_html_root']) ? $setting ['create_to_html_root']: false;
				// 检查是否生成到根目录
	...
	public function remove() {
		...
		if (isset ( $_POST ['dosubmit'] )) {
			...
			include $this->view ( 'category_remove' );
		}
	}
}