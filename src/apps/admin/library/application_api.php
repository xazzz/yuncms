 ...
 * @version $Id: application_api.php 668 2013-07-30 03:08:49Z 85825770@qq.com $
 */
	...
	public function uninstall($application) {
		...
		if (is_array ( $models ) && ! empty ( $models )) {
			foreach ( $models as $m ) {
				...
				$this->sql_execute ( $sql );
				// @unlink ( WEKIT_PATH . 'model' . DIRECTORY_SEPARATOR . $m .
				// '_model.php' );
			}
		}
		/*
        if (file_exists ( WEKIT_PATH . 'language' . DIRECTORY_SEPARATOR . C ( 'config', 'lang' ) . DIRECTORY_SEPARATOR . $this->application . '.php' )) {
            @unlink ( WEKIT_PATH . 'language' . DIRECTORY_SEPARATOR . C ( 'config', 'lang' ) . DIRECTORY_SEPARATOR . $this->application . '.php' );
        }
        if (is_dir ( WEKIT_PATH . 'template' . DIRECTORY_SEPARATOR . C ( 'template', 'name' ) . DIRECTORY_SEPARATOR . $this->application )) {
            Folder::rm ( WEKIT_PATH . 'template' . DIRECTORY_SEPARATOR . C ( 'template', 'name' ) . DIRECTORY_SEPARATOR . $this->application );
        }
        $viewpath = WEKIT_PATH . 'template' . DIRECTORY_SEPARATOR . C ( 'template', 'name' ) . DIRECTORY_SEPARATOR;
        if (file_exists ( $viewpath . 'config.php' )) {
            $keyid = 'template|' . C ( 'template', 'name' ) . '|' . $this->application;
		 ...
		 * if (file_exists ( WEKIT_PATH . 'language' . DIRECTORY_SEPARATOR . C
		 * ( 'config', 'lang' ) . DIRECTORY_SEPARATOR . $this->application .
		 * '.php' )) { @unlink ( WEKIT_PATH . 'language' . DIRECTORY_SEPARATOR
		 * . C ( 'config', 'lang' ) . DIRECTORY_SEPARATOR . $this->application .
		 * '.php' ); } if (is_dir ( WEKIT_PATH . 'template' .
		 * DIRECTORY_SEPARATOR . C ( 'template', 'name' ) . DIRECTORY_SEPARATOR
		 * . $this->application )) { Folder::rm ( WEKIT_PATH . 'template' .
		 * DIRECTORY_SEPARATOR . C ( 'template', 'name' ) . DIRECTORY_SEPARATOR
		 * . $this->application ); } $viewpath = WEKIT_PATH . 'template' .
		 * DIRECTORY_SEPARATOR . C ( 'template', 'name' ) . DIRECTORY_SEPARATOR;
