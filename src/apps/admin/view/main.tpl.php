...
<?php } ?>
<?php if(C('config','debug') != false) {?>
<?php echo L('main_safety_debug')?><br />
<?php } ?>
<?php if(C('log','log_threshold') != 0) {?>
<?php echo L('main_safety_errlog')?><br />
<?php } ?>
	<div class="bk20 hr">
				<hr />
			</div>
<?php if(C('system','execution_sql')) {?>
...
<?php } ?>
	...
	<div class="col-2 lf mr10" style="width: 48%">
		...
		<div class="content" >
	<span onclick="art.dialog.open('?app=admin&controller=index&action=public_our', {id:'map',title:'<?php echo L('main_our')?>', width:'400px', height:'300px', lock:true});void(0);"><?php echo L('main_copyright')?><?php echo $product_copyright?></span><br />
	...
	<?php echo L('main_product_site')?><a href="http://www.tintsoft.com/"
				target="_blank">http://www.tintsoft.com/</a> <br />
	<?php echo L('main_product_bbs')?><a href="http://bbs.tintsoft.com/"
				target="_blank">http://bbs.tintsoft.com/</a>
		</div>
	</div>
