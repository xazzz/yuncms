 ...
 * @version $Id: PaymentController.php 713 2013-08-02 02:20:33Z 85825770@qq.com $
 */
class PaymentController extends Web_Admin {
	...
	public function pay_list() {
		...
		if (isset ( $_GET ['dosubmit'] )) {
			extract ( $_GET ['info'] );
			if ($trade_sn) $where ['trade_sn'] = array ('like',"%$trade_sn%" );
		...
		$infos = $this->account_db->where ( $where )->order ( 'addtime DESC,id DESC' )->listinfo ( $page, 20 );
		$pages = $this->account_db->pages;
		$number = count ( $infos );
		include $this->view ( 'pay_list' );
	}
	...
	public function pay_stat() {
		...
		if (isset ( $_GET ['dosubmit'] )) { // 查询统计
			...
			if ($status) $where ['status'] = array ('like',"%$status%" );
			$infos = $this->account_db->where ( $where )->select ();
			$num = count ( $infos );

			$amount_num = $amount = $amount_succ = $amount_num_succ = 0;
			$point_num = $point = $point_succ = $point_num_succ = 0;
			foreach ( $infos as $_v ) {
				if ($_v ['type'] == 1) {
					$amount_num ++;
					$amount += $_v ['money'];
		...
		$total_amount_num = $total_amount = $total_amount_succ = $total_amount_num_succ = 0;
		$total_point_num = $total_point = $total_point_succ = $total_point_num_succ = 0;
	...
	public function discount() {
		if (isset ( $_POST ['dosubmit'] )) {
			...
			extract ( $infos );
			include $this->view ( 'pay_discount' );
		}
	}
