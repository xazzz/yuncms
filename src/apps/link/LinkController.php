 ...
 * @version $Id: LinkController.php 629 2013-07-29 06:20:37Z 85825770@qq.com $
 */
defined ( 'IN_YUNCMS' ) or exit ( 'No permission resources.' );

class LinkController extends Web_Admin {
	...
	public function check_register() {
		if (isset ( $_POST ['dosubmit'] )) {
			...
			$pages = $this->db->pages;
			$big_menu = big_menu ( '?app=link&controller=link&action=add', 'add', L ( 'link_add' ), 700, 450 );
			include $this->view ( 'check_register_list' );
		}
	}
	...
	public function add_type() {
		if (isset ( $_POST ['dosubmit'] )) {
			...
			$big_menu = big_menu ( '?app=link&controller=link&action=add', 'add', L ( 'link_add' ), 700, 450 );
			include $this->view ( 'link_type_add' );
		}
	}
	...
	public function edit_type() {
		if (isset ( $_POST ['dosubmit'] )) {
			...
			extract ( $info );
			include $this->view ( 'link_type_edit' );
		}
	}
	...
	public function list_type() {
		...
		$big_menu = big_menu ( '?app=link&controller=link&action=add', 'add', L ( 'link_add' ), 700, 450 );
		include $this->view ( 'link_list_type' );
	}
