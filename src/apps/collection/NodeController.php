...
class NodeController extends Web_Admin {
	private $db;
	...
	public function col_url_list() {
		...
		if ($data = $this->db->getby_nodeid ( $nodeid )) {
			...
			if ($total_page > 0) {
				...
				include $this->view ( 'col_url_list' );
			} else {
	...
	public function import() {
		...
		include $this->view ( 'import_program' );
	}
	...
	public function import_program_add() {
		...
		$show_header = true;
		include $this->view ( 'import_program_add' );
	}
	...
	public function import_content() {
		...
		if ($type == 'all') {
			...
			$data = $collection_content_db->where ( array ('nodeid' => $nodeid,'status' => 1 ) )->field ( 'id, data' )->order ( $order )->limit(20)->select ();
		} else {
	...
	public function public_url() {
		...
		$show_header = true;
		include $this->view ( 'node_public_url' );
	}
