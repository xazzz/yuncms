...
<fieldset>
    ...
    <tr>
		...
		<td class="y-bg"><?php echo Form::radio(array('0'=>L('model_configuration'), '1'=>L('custom_sql'), '2'=> L('block')), isset($type) ? $type : 0, 'name="type" onclick="location.href=\''.Base_Request::get_url().'&type=\'+this.value"')?></td>
	</tr>
  <?php if ($type==0) :?>
    <tr>
		<th><?php echo L('select_model')?>：</th>
		<td class="y-bg"><?php echo Form::select($applications, $application, 'name="application" id="application" onchange="location.href=\''.Base_Request::get_url().'&application=\'+this.value"')?><script type="text/javascript">$(function(){$("#application").formValidator({onshow:"<?php echo L('please_select_model')?>",onfocus:"<?php echo L('please_select_model')?>"}).inputValidator({min:1, onerror:'<?php echo L('please_select_model')?>'});});</script></td>
	</tr>
  <?php if ($application):?>
    <tr>
		<th><?php echo L('selectingoperation')?>：</th>
		<td class="y-bg"><?php echo Form::radio($html['do'], $do, 'name="do" onclick="location.href=\''.Base_Request::get_url().'&do=\'+this.value"')?></td>
	  </tr>
	  <?php endif;?>
