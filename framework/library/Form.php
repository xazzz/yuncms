 ...
 * @version $Id: Form.php 644 2013-07-29 08:57:25Z 85825770@qq.com $
 */
class Form {
	...
	public static function editor($textareaid = 'content', $toolbar = 'basic', $application = '', $catid = '', $color = '', $allowupload = 0, $allowbrowser = 1, $alowuploadexts = '', $height = 200, $disabled_page = 0, $allowuploadnum = '10') {
		...
		$str .= "<script type=\"text/javascript\">\r\n";
		$str .= "UE.getEditor('" . $textareaid . "',{" . join ( ",", $opt ) . "});";
		$str .= '</script>';
		return $str;
	}
	...
	public static function upfiles($name, $id = '', $value = '', $moudle='', $catid='', $size = 50, $class = '', $ext = '', $alowexts = '',$file_setting = array() ) {
		...
		if(!$alowexts) $alowexts = 'rar|zip|7z|tgz|tar|cab|gz|bz2|bzip2|gzip|z|xzr';
		if(!defined('IMAGES_INIT')) {
			$str = '<script type="text/javascript" src="'.JS_PATH.'swfupload/swf2ckeditor.js"></script>';
			define('IMAGES_INIT', 1);
		}
	...
	public static function select_template($style, $application, $id = '', $str = '', $pre = '') {
		$templatedir = WEKIT_PATH . 'template' . DIRECTORY_SEPARATOR . $style . DIRECTORY_SEPARATOR . $application . DIRECTORY_SEPARATOR;
		$confing_path = WEKIT_PATH . 'template' . DIRECTORY_SEPARATOR . $style . DIRECTORY_SEPARATOR . 'config.php';
