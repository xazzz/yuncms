 ...
 * @version $Id: Format.php 648 2013-07-29 09:56:28Z 85825770@qq.com $
 */
class Format {
	 ...
	 * @return string
	 */
	public static function format_data($format = null, $date_time = null) {
	...
	public static function date($timestamp, $showtime = 0) {
		...
		$lang = C ( 'config', 'lang' );
		if ($lang == 'zh-cn') {
			$str = $showtime ? self::format_data ( 'Y-m-d H:i:s', $times ) : self::format_data ( 'Y-m-d', $times );
		} else {
			$str = $showtime ? self::format_data ( 'm/d/Y H:i:s', $times ) : self::format_data ( 'm/d/Y', $times );
		}
		return $str;
	}
	...
	public static function get_week($week = null) {
		$week = $week ? $week : self::format_data ( 'w' );
	...
	public static function is_leap_year($year = null) {
		$year = $year ? $year : self::format_data ( 'Y' );
	...
	public static function get_period_of_time($hour = null) {
		$hour = $hour ? $hour : self::format_data ( 'G' );
		$period = null;
		if ($hour >= 0 && $hour < 6) {
			$period = '凌晨';
	...
	public static function year_to_chinese($year = null, $flag = false) {
		$year = $year ? intval ( $year ) : self::format_data ( 'Y' );
	...
	public static function date_info($type, $date = null) {
		$year = self::format_data ( 'Y', $date );
		$month = self::format_data ( 'm', $date );
		$day = self::format_data ( 'd', $date );
		$result = null;
		switch ($type) {
			case 'SX' :
	...
	public static function date_add($interval, $value, $date_time = null, $format = null) {
		$date_time = $date_time ? $date_time : self::format_data ();
		...
		return self::format_data ( $format, mktime ( $date ['hours'], $date ['minutes'], $date ['seconds'], $date ['mon'], $date ['mday'], $date ['year'] ) );
	}
	...
	public static function first_day_of_year($year = null, $format = 'Y-m-d') {
		$year = $year ? $year : self::format_data ( 'Y' );
		return self::format_data ( $format, mktime ( 0, 0, 0, 1, 1, $year ) );
	}
	...
	public static function last_day_of_year($year = null, $format = 'Y-m-d') {
		$year = $year ? $year : self::format_data ( 'Y' );
		return self::format_data ( $format, mktime ( 0, 0, 0, 1, 0, $year + 1 ) );
	}
	...
	public static function first_day_of_month($month = null, $year = null, $format = 'Y-m-d') {
		$year = $year ? $year : self::format_data ( 'Y' );
		$month = $month ? $month : self::format_data ( 'm' );
		return self::format_data ( $format, mktime ( 0, 0, 0, $month, 1, $year ) );
	}
	...
	public static function last_day_of_month($month = null, $year = null, $format = 'Y-m-d') {
		$year = $year ? $year : self::format_data ( 'Y' );
		$month = $month ? $month : self::format_data ( 'm' );
		return self::format_data ( $format, mktime ( 0, 0, 0, $month + 1, 0, $year ) );
	}
	...
	public static function get_day_range_in_between_date($start_date_time, $end_date_time, $sort = false, $format = 'Y-m-d') {
		...
		$date_arr = array ();
		for($i = 0; $i <= $num; $i ++) {
			$date_arr [] = self::format_data ( $format, $start_date_time + 86400 * $i );
		}
