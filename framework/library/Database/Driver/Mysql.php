 ...
 * @version $Id: Mysql.php 733 2013-08-06 15:30:22Z 85825770@qq.com $
 */
class Database_Driver_Mysql extends Base_DB {
	...
	public function __construct($config = '') {
		if (! extension_loaded ( 'mysql' )) {
			throw new Base_Exception ( 'suppert does not exist.' . ':mysql' );
		}
		if (! empty ( $config )) {
			$this->config = $config;
		}
	...
	public function connect() {
		...
		if (! $this->link) {
			throw new Base_Exception ( "Couldn't connect to SQL Server on " . $this->config ['hostname'] );
		}
		if ($this->version () > '4.1') {
			// 设置数据库编码
	...
	public function select_db($database = null) {
		if (! is_null ( $database ) && ! @mysql_select_db ( $database, $this->link )) {
			throw new Base_Exception ( 'Cannot use database ' . $database );
			return false;
		}
