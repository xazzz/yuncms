 ...
 * @version $Id: Sqlsrv.php 623 2013-07-29 03:40:03Z 85825770@qq.com $
 */
class Database_Driver_Sqlsrv extends Base_DB {
	...
	public function __construct($config = '') {
		if (! function_exists ( 'sqlsrv_connect' )) {
			throw new Base_Exception ( 'suppert does not exist.:sqlsrv' );
		}
		if (! empty ( $config )) {
			$this->config = $config;
		}
	...
	public function error($result = true) {
		...
		if ($result) {
			...
			throw new Base_Exception ( $this->error );
		}
		return $this->error;
	}
