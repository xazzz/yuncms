 ...
 * @version $Id: Mssql.php 623 2013-07-29 03:40:03Z 85825770@qq.com $
 */
class Database_Driver_Mssql extends Base_DB {
	...
	public function __construct($config = '') {
		if (! extension_loaded ( 'mssql' )) {
			throw new Base_Exception ('suppert does not exist.' . ':mssql' );
		}
		if (! empty ( $config )) {
			$this->config = $config;
		}
	...
	public function connect() {
		...
		if (! $this->link) {
			throw new Base_Exception ( "Couldn't connect to SQL Server on $host" );
		}
	...
	public function select_db($database = null) {
		if (! is_null ( $database ) && ! @mssql_select_db ( $database, $this->link )) {
			throw new Base_Exception ( "Couldn't open database  " . $database );
			return false;
		}
	 ...
	 * @param $key 俺键名排序
	 * @param int $mode
	 *        	获得结果集的模式MSSQL_ASSOC，MSSQL_NUM 和 MSSQL_BOTH
	 * @return array
	 */
