 ...
 * @version $Id: DB.php 623 2013-07-29 03:40:03Z 85825770@qq.com $
 */
abstract class Base_DB {
	...
	public function parse_sql($sql, $options = array()) {
		$sql = str_replace ( array ('%TABLE%','%DISTINCT%','%FIELD%','%JOIN%','%WHERE%','%GROUP%','%HAVING%','%ORDER%','%LIMIT%','%UNION%','%COMMENT%' ), array ($this->parse_table ( $options ['table'] ),$this->parse_distinct ( isset ( $options ['distinct'] ) ? $options ['distinct'] : false ),$this->parse_field ( ! empty ( $options ['field'] ) ? $options ['field'] : '*' ),$this->parse_join ( ! empty ( $options ['join'] ) ? $options ['join'] : '' ),$this->parse_where ( ! empty ( $options ['where'] ) ? $options ['where'] : '' ),$this->parse_group ( ! empty ( $options ['group'] ) ? $options ['group'] : '' ),$this->parse_having ( ! empty ( $options ['having'] ) ? $options ['having'] : '' ),$this->parse_order ( ! empty ( $options ['order'] ) ? $options ['order'] : '' ),$this->parse_limit ( ! empty ( $options ['limit'] ) ? $options ['limit'] : '' ),$this->parse_union ( ! empty ( $options ['union'] ) ? $options ['union'] : '' ),$this->parse_comment ( ! empty ( $options ['comment'] ) ? $options ['comment'] : '' ) ), $sql );
		return $sql;
	}
			...
			foreach ( $where as $key => $val ) {
				...
				if (0 === strpos ( $key, '_' )) {
					...
					if (! preg_match ( '/^[A-Z_\|\&\-.a-z0-9\(\)\,]+$/', trim ( $key ) )) {
						throw new Exception ( 'Express error:' . $key );
					}
					// 多条件支持
		...
		if (is_array ( $val )) {
			if (is_string ( $val [0] )) {
				if (preg_match ( '/^(EQ|NEQ|GT|EGT|LT|ELT)$/i', $val [0] )) { // 比较运算
					...
					throw new Exception ( 'Express error:' . $val [0] );
				}
			} else {
				$count = count ( $val );
