 ...
 * @version $Id: kernel.php 693 2013-07-30 07:15:33Z 85825770@qq.com $
 */
...
class Core {
	...
	public static function autoLoad($className, $path = '') {
		if (! empty ( $path )) { // 手动加载
			...
			$_classPath = self::get_real_path ( str_replace ( '_', '.', $className ), FW_PATH . 'library' . DIRECTORY_SEPARATOR );
	...
	public static function import($filePath, $folder = '') {
		...
		$_classPath = self::get_real_path ( $filePath, $folder );
	 ...
	 */
	public static function get_real_path($filePath, $base = '') {
	...
	public static function _shutdownHandle() {
		if (($errno = error_get_last ()) && $errno ['type'] == E_USER_ERROR) {
